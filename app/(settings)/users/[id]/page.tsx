          ...
          <div className='space-y-4'>
            <div className='flex flex-col sm:flex-row items-center gap-4 pb-4 border-b border-gray-100 dark:border-slate-700'>
              ...
              <div className="text-center sm:text-left">
                <h1 className='text-xl font-semibold text-slate-800 dark:text-white'>Amine Elhasbi</h1>
                <p className="text-sm text-slate-500 dark:text-gray-400">Student</p>
              </div>
            </div>
            
            <div className="space-y-3">
              {profileDetails.map((data) => (
                <div key={data.label} className='flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3 py-2'>
                  <p className='text-sm font-semibold text-slate-700 dark:text-gray-300 min-w-[80px]'>{data.label}:</p>
                  <p className='text-sm text-slate-600 dark:text-gray-400 break-words'>{data.value}</p>
                </div>
              ))}
            </div>
